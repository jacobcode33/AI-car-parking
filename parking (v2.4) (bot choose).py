# TO DO LIST:
# new bot genre "redo" tests the best bots to then when scores come in the score becomes the average between them
# overlap of car and parking spot for more points
# make more complex with hidden layer
# when both left and right keys are pressed it appears that neither are pressed
...
starttime = time.time()
bots = 20
rounds = 5 # MUST STAY 5 FOR RELIABILITY
variety = 0.3
runs = 25
database = 1
for run in range (0, runs):
     ...
     def makebots(genre, randomness):
          ...
          for o in range (0, bots):
               ...
               if genre == "best":
                    ...
                    data = [row for row in csv.reader(file)]
                    for i in range (0, 36):
                         stored = (data[bestno][2])
                         stored = (stored.strip("][").split(", "))
                         weight.append (float(stored[i])+random.uniform(-randomness,randomness))
                    for i in range (0,6):
                         stored = (data[bestno][3])
                         stored = (stored.strip("][").split(", "))
                         bias.append (float(stored[i])+random.uniform(-randomness,randomness))
                    file.close()
               ...
               if genre == "redo":
                    ...
                    for i in range (0, 36):
                         ...
                         stored = (stored.strip("][").split(", "))
                         weight.append (float(stored[i]))
                    for i in range (0,6):
                         stored = (data[bestnos[o]][3])
                         stored = (stored.strip("][").split(", "))
                         bias.append (float(stored[i]))
                    file.close()
     ...
     def record():
          if (len(bestnos)) > 0:
               ...
               for who in range (0, bots):
                    data.writerow([makename(), scores[who], weights[who], biases[who]])
               file.close()
