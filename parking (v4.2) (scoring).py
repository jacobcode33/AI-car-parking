...
def sense(who):
     ...
     sensors[6] = ((math.sqrt((0.5-(((goala- cara[who])%90) / 90))**2))+0.5)
...
def mark():
     ...
     global cara
     for i in range(0, bots):
          startdis = (math.sqrt((goalx-startx)**2 + (goaly-starty)**2))
          nowdis = (math.sqrt((goalx-carx[i])**2 + (goaly-cary[i])**2))
          moved = (startdis - nowdis) / 3##
          moved = ((startdis - nowdis) / startdis) * 80
          parked = 0
          if over(i) == True:
               parked = parked + 60
               if round((speed[i]**2), 3) == 0:
                    parked = parked + 20
          parked = parked / ((speed[i]**2)+ 1)
          parked = parked / ((math.sqrt((0.5-(((goala- cara[i])%90) / 90))**2))+0.5)
...
def murder():
     ...
     if len(data) > keep:
          ...
          for i in range (0, len(data)):
               List.append(float(data[i][1]))
          for i in range (0, len(data)):
               bestno.append(List.index(max(List)))
               List[List.index(max(List))] = -9999
          file.close()
          ...
          for who in range (0, keep):
               scribe.writerow([data[bestno[who]][0], data[bestno[who]][1], data[bestno[who]][2], data[bestno[who]][3], data[bestno[who]][4], data[bestno[who]][5], data[bestno[who]][6]])
     file.close()
...
def record(genre):
     if genre == "redo":
          ...
          for who in range (0, bots):
               data.writerow([makename(), scores[who], wsone[who], bsone[who], wstwo[who], bstwo[who], 1])
          file.close()
     file = open ("database"+str(database)+".csv", "rt", newline = "")
     file.close()
     ...
     for obstacles in range (0, rounds):
          ...
          if progress > 0:
               ...
               if timeleft > 60:
                    ...
                    timeleft = str(round(timeleft))+ " secs"
          w = False
          s = False
          r = False
          a = False
          ...
          while count > 0:
               ...
               while who < bots:
                    crash = False
                    sensors = [9999, 9999, 9999, 9999, 9999, 9999, 9999]
                    sense(who)
                    botchoose(who)
                    #humanchoose()
                    check(who)
          ...
          mark()
     if plot == False:
          print (str(round((progress*100), 1))+"%", "                    ", timeleft)
     if genre == "random":
          qrandom = qrandom + 1
     elif genre == "redo":
          qredo = qredo + 1
