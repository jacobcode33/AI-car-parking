# TO DO LIST:
# handbrake, point multiplier for finishing with handbrake
# lose points for handbrake at speed
# cannot undo handbrake or move with handbrake
...
def murder():
     print ("press y to confirm the murder of the least impressive 50% of bots: ")
     ...
     file.close()
     avgscore = 0
     for who in range (0, len(data)):
          avgscore = ((avgscore*who) + float(data[who][1]))/(who+1)
     file = open ("database"+str(database)+".csv", "w", newline = "")
     scribe = csv.writer (file, delimiter=',')
     for who in range (0, len(data)):
          if float(data[who][1]) > avgscore:
               scribe.writerow([data[who][0], data[who][1], data[who][2], data[who][3], data[who][4], data[who][5], data[who][6]])
     file.close()
